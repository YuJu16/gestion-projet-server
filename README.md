...
| GET | /api/projects/:id/tasks | Tâches du projet (filtre ?status=) | oui |
| POST | /api/projects/:id/tasks | Créer une tâche | oui |
| PUT | /api/projects/:id/tasks/:taskId | Modifier une tâche | oui |
| DELETE | /api/projects/:id/tasks/:taskId | Supprimer une tâche | oui |
| GET | /api/projects/:id/participants | Liste des participants | oui |
| POST | /api/projects/:id/participants | Ajouter un participant (email) | oui (owner) |
...
- Un participant peut se retirer lui-même d'un projet (pas juste le owner) - ajouté en plus de ce qui était demandé au départ.
